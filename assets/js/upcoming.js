...
var divcards = document.getElementById("contcardsIdUpcoming"); // CREO VARIABLE DONDE SE VAN A ALOJAR LOS VALORES QUE ME DEVUELVA LA FUNCION, POR ID
...
currentDate = cardArray.currentDate;
...
function showBox(){
let categories = cardArray.events.map(function (evento) {
...
checkBoxCall[i].addEventListener("click", (evento)=>{ // ESCUCHO EL EVENTO CLICK EN CADA ITERACION DEL BUCLE FOR
  ...
  if (checked.length == 0 && inputValue == ""){
    filteredSearchArray = cardArray.events
  }
  else if (checked.length > 0 && inputValue == ""){
    filteredSearchArray = cardArray.events.filter(everyCardInfo => checked.includes(everyCardInfo.category)) 
    // console.log(filteredSearchArray);
  } else if (checked.length > 0 && inputValue !== ""){
    filteredSearchArray = cardArray.events.filter(everyCardInfo => checked.includes(everyCardInfo.category)).filter(everyCardInfo => everyCardInfo.name.toLowerCase().trim().includes(inputValue)) 
  }
  upcomingEventsCards(filteredSearchArray);
})
...
// 2 . CHECKBOXES SIN CLICKEAR Y TEXTO INGRESADO POR IMPUT SEARCH
  ...
  filteredSearchArray = cardArray.events.filter(everyCardInfo => everyCardInfo.name.toLowerCase().trim().includes(inputValue)) 
}
...
function upcomingEventsCards(arrayInfo) {
    let contenidoCard = "";
    for (let i=0; i<arrayInfo.length;i++){
        if (currentDate > arrayInfo[i].date) {
            contenidoCard += 
    ...
    divcards.innerHTML=contenidoCard;
